...
void ASpawnPoint::SpawnEnemy(UDataTable* CodeTable, int32 Amount, float Delay)
{
	GetWorldTimerManager().SetTimer(SpawnTimerHandle, [=]()
		{
			UE_LOG(LogTemp, Warning, TEXT("Spawn enemy from spawn point"));
			//Spawn next if any
			if (Amount == 0) return;
...
{
	UE_LOG(LogTemp, Warning, TEXT("%s in OnEnemyDestroyed: "), *Actor->GetName());
	AEcmaEnemyCharacter* Enemy = Cast<AEcmaEnemyCharacter>(Actor);
	EnemyGroup.Remove(Enemy);

	if (EnemyGroup.Num() == 0)
	{
		UE_LOG(LogTemp, Warning, TEXT("Enemies in group destroyed."));
		WaveNum++;
